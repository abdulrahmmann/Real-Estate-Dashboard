        ...
        <Fragment>
            ...
            <div className={`my-5`}>
                <div className={`bg-whiteColor rounded-xl px-7 py-5 w-fit flex items-center gap-4 relative`}>
                    <button
                        className={`text-sm transition-all ${tabClicked === "All Reviews" ? 'font-semibold text-primaryColor relative tabActive' : 'font-normal text-secondaryTextColor'}`}
                        onClick={() => setTabClicked("All Reviews")}>
                        All Reviews
                    </button>

                    <button
                        className={`text-sm transition-all ${tabClicked === "Published" ? 'font-semibold text-primaryColor relative tabActive' : 'font-normal text-secondaryTextColor'}`}
                        onClick={() => setTabClicked("Published")}>
                        Published
                    </button>

                    <button
                        className={`text-sm transition-all ${tabClicked === "Deleted" ? 'font-semibold text-primaryColor relative tabActive' : 'font-normal text-secondaryTextColor'}`}
