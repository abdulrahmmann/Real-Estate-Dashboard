...
export const UserProfileDropdown = ({ onEditProfile }: { onEditProfile: () => void }) => {
    return (
        <div
            className={`w-[150px] bg-[#FCFCFC] p-2.5 rounded-xl absolute top-5 z-50 right-0 shadow-sm drop-shadow-sm`}
            style={{ boxShadow: "0px 16px 30px 0px rgba(135, 134, 134, 0.1)" }}
        >
            <div className={`flex flex-col`}>
                <button
                    onClick={onEditProfile}
                    className={`p-2.5 flex items-center gap-2.5 hover:bg-neutral-100 rounded-lg transition-all`}
                >
